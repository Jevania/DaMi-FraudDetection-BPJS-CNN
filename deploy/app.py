...
def main():
    ...
    with col2:
        ...
        KDKC = st.number_input(
            "BPJS Health Care Office branch area code", 1, 2606)
        ...
        jkpst_list = ["L", "P"]
        JKPST = st.selectbox(
            "Select gender: 0 -> Male || 1 -> Female", jkpst_list)
        JKPST = 1 if JKPST == "P" else 0

        UMUR = st.number_input(
            "JKN-KIS participant age at the time of hospital service", 0, 109)
        JNSPELSEP = st.number_input(
            "Level of service: 1 -> inpatient || 2 -> outpatient", 1, 2)
        LOS = st.number_input(
            "Length of time the JKN-KIS participant was hospitalized", 0, 592)
        ...
        if st.button('Predict'):
            ...
            col1.success(
                f"{prediction_result} is the prediction result for your BPJS claim")
      # code for html
